...
const renderPage = () => {
  const router = createMemoryRouter(
    [{ path: '/', element: <DatenschutzPage />, errorElement: <RouteErrorElement /> }],
    { initialEntries: ['/'] },
  )
...
describe('DatenschutzPage', () => {
  ...
  it('renders the document content through MarkdownView when set', () => {
    mocks.data = { content: '# We log nothing\n\nNo tracking cookies.', updatedAt: null }
    renderPage()
    expect(screen.getByRole('heading', { name: 'We log nothing' })).toBeInTheDocument()
